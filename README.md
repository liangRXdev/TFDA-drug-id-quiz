...
三種難度**各自**記錄最高分與最長連對，存在瀏覽器 `localStorage`，起始頁可以看到與清除。
...
教學藥師在起始頁按「教學藥師：產生院內清單連結」，貼上院內的許可證字號（每行一個，
...
- 起始頁可**切回全題庫**（會把清單從這台瀏覽器移除，要再用得重新開連結）
...
```bash
npm test                 # engine、難度分級、閃卡、UI 接線、娛樂性強化、錯題再戰、院內清單、資料管線與 SW 界線（610 項）
