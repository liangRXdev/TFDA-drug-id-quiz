...
- **SW `CACHE` 版本維持 `v2`**。規格〔2-7〕寫「每批無條件升版」，但本輪是**批次 A 自身的覆審修正**，
  與 `35d05d7` 同屬一個未發布的 PR，`v2` 從未進過任何使用者的 cache，升 `v3` 只會讓版本號與批次標籤脫鉤。
  若判定本輪算獨立一批，改 `sw.js:27` 與 `tests/sw.test.mjs:134` 兩處即可。
