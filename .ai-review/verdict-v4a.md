...
| 項目 | 修法 | 證據 |
|---|---|---|
| **SC-2** | 採 verdict 的修法 (a)。實測確認來源**沒有** `Last-Modified` 也**沒有** `ETag`，但 ZIP 中央目錄的 DOS 日期欄位（`off+14`）標 `2026-08-03`，**比下載當下早 3 天**——是來源端的資料產生日，不是隨請求重編的時間戳，因此可放進 `meta.source_version` 而不破壞 B7。格式取 `YYYY-MM-DD`，與 `plan.md:228` 的範例一致，故**規格文字無須修訂**。ZIP 無合法日期時**不寫欄位**（寫假值比缺欄位更糟，前端的 `if (m.source_version)` 本來就容許缺席） | 實跑（**輸出導向 `.probe/pool.json`，非 `data/pool.json`**）：`資料版本 2026-08-03`，該暫存產物的 meta 出現此欄位。**已提交的 `data/pool.json` 本輪未重建，因此使用者端仍看不到——見下方「未重建 `data/pool.json`」與 `verdict-v4b.md` 的 SC-2** |
...
> **〔2026-08-07 後記〕歸因乾淨與功能送達是兩件事。**
> 上面這個決定本身站得住，但它的**後果**當時沒寫出來：C6 要求顯示的資料版本，
> 在合併之後對使用者**依然靜默不存在**——而這正是 SC-2 原本的病徵。
> `verdict-v4b.md` 的 SC-2 把這一項升為合併阻塞。
> 裁示：PR #3 維持純程式碼，**合併後立即手動觸發 `update-pool` workflow**
> 由正式管線產出並推回 `main`，兩個目的都達成。
> 交付檢查清單見 `verdict-v4b.md` 的「合併後必做」。
