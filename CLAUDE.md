...
**產連結頁刻意放在 `index.html` 內而不是獨立頁面。** 理由不是省事：
C17／C25／C27 這三條靜態 CSS 契約掃的是 `index.html`，
另開一頁等於讓那頁的樣式與動畫**逃出掃描範圍**——正是本 repo 記過的
「假綠燈第三種形態：斷言正確但沒掃到違規發生的位置」。
真要拆頁，得先把三個掃描器的範圍一起擴大，否則新頁不受任何契約約束。
`excluded.json` 只在打開該區塊時才抓，D47 的「不得拖垮既有功能」由 lazy fetch 滿足。

**院內版的短卷測試用 L3，不是 L1。** 真實 pool 上，L1 在 13–22 個可出題鍵的區間
**過得了 K 門檻卻組不出整卷**（`eligibleKeys()` 是逐鍵局部判定，整卷受 H2 約束）——
實測 n=14…23 的隨機子集全部 `QUIZ_ASSEMBLY_FAILED`。
這正是 D38.2 存在的理由，不是 bug。要驗短卷就用不需誘答的 L3。
